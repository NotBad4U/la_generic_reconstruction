...
% \newcommand{\changed}[2]{\todo[#1]{#1!}\textcolor{blue}{#2}}

% Critical pair diagram
\newcommand\cp[6][\linewidth]{
\begin{minipage}[c]{#1}
% \centering
\begin{tikzpicture}[scale=.9, node distance=3em]
\node[scale=.9] (root) { \scriptsize $\overline{#2}$ };
\node[scale=.9] (left) at ($(root.south) + (-3,-1)$) {\scriptsize $#3$};
\node[scale=.9] (right) at ($(root.south) + (3,-1)$) {\scriptsize $#4$};
