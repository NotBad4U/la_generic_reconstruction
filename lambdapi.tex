...
Figure~\ref{fig:arith-ops} introduces operations on these types, including addition $+$ and comparison $\doteq$ over $\bb{Z}$.\todo[sm]{Must also explain definitions of \tt{add}, \tt{sub}, \tt{cmp}. If there is not enough space, describe them in the text and move their definitions to the appendix. -- OK, I see that add and sub appear in the appendix, and presumably cmp is what's called compare in the appendix. Adding a sentence is enough.}
%Similarly for $\bb{P}$ with the infix function $\tt{cmp}: \bb{P} \ra \bb{P} \ra \tt{Comp}$.
...
For example, $1 < 2 \hookrightarrow \tt{istrue}(\tt{isLt}(1 \doteq 2)) \hookrightarrow \tt{istrue}(\tt{isLt}(\tt{Lt})) \hookrightarrow \tt{istrue}(\tt{true}) \hookrightarrow \top$.\todo[sm]{Strictly speaking, $1$ and $2$ have not been introduced, and it is not clear how $1 \doteq 2$ is evaluated.}
...
\begin{definition}%[Function $\mathcal{E}$ translating SMT expressions]
  The definition of $\E{e}$ mapping expressions from SMT to $\lpm$ is as follows.\todo[sm]{This doesn't cover all cases.}
  \begin{itemize}
