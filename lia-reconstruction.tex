...
We have to show that $\den{[t_1 \oplus t_2]} = \den{(t_1)} + \den{(t_2)}$.
By the induction hypothesis, we have:
\[
	\den{[t_1]} = \den{t_1} \quad \text{and} \quad \den{[t_2]} = \den{t_2}.
\]
Hence,
\[
	\den{[t_1 \oplus t_2]} = \mathop{\den{[t_1] \oplus [t_2]}} = \mathop{\den{[t_1]}} + \den{[t_2]} = \mathop{\den{(t_1 \oplus t_2)}}.
\]
It remains to show that:
\[
	\den{[t_1 \oplus t_2]} = \mathop{\den{[t_1] \oplus [t_2]}}.
\]

This follows from the fact that $[t_1 \oplus t_2]$ is the canonical (i.e., sorted) form of the multiset of subterms in $t_1$ and $t_2$,
