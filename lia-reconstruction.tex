...
Lastly, the constructor $\add{}{}$ represents the addition \changed{sm}{of} two elements of $\bb{G}$.
...
Terms are ordered such that $\tt{cst}(c_1) \leq \tt{cst}(c_2) < \var{p}{x}$ for any constants $c_1 \leq c_2$ and any variable term $\var{p}{x}$.
...
We make use of Lem.~\ref{lem:conv} to embed $\bb{Z}$-terms into $\bb{G}$ \changed{sm}{for normalization} and subsequent comparison.
...
\todo[sm]{We don't have space for showing the full translation of the running example, but perhaps we could show the translation of a single rule?}
