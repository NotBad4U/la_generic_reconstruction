...
Proof by reflection is a technique for writing certified automation procedures. It reduces the validity of a logical statement to a symbolic computation.
...
The base type for this group's elements is $\bb{G}: \type$. The unary operator $\tt{cst}$ denotes a constant from $\bb{Z}$.
A $\tt{var}$ constructor for "catch-all" case for subexpressions that we cannot model. These subexpressions correspond to actual variables in $\bb{Z}$.
...
Lastly, the constructor $\tt{add}$ represents the addition between two elements of \cref{fig:grp}.
