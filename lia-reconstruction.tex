...
&|~\tt{var}: \bb{Z} \ra \bb{Z} \ra \bb{G} & & \mathop{\reify{\tt{ZPos}}} c \re \cst{c} & & \den{\opp{x}} \mathrel{\re}  \mathop{\sim (\den{x})} \\
...
\todo[sm]{In fig.5, we have $\var{x}{c}$, whereas before we had $\var{c}{x}$ -- which one is it? Please be uniform.}
\begin{figure}
\begin{align*}
...
The transformation to canonical form implemented in Lambdapi ensures that sum expressions of the form $\add{\var{c_1}{x_1}}{\add{\var{c_2}{x_2}}{\add{\dots}{\var{c_n}{x_n}}}}$\todo[sm]{Should also allow for constants in these sums.} will be normalized such that any pair of terms $\var{p}{x}$ and $\var{q}{x}$ involving the same variable $x$ are placed next to each other.
...
Let $\leq$ be a total order\todo[sm]{Before you said that the order was built into Lambdapi, now it sounds as if the user could influence the order?} on $\bb{G}$-terms defined as follows:
